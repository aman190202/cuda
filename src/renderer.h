...
{
    // Parameters
    const float base_step_size = 1.0f;
    ...
    const float atten_k = 0.1f;
    const float min_step_size = 0.1f;
    const float max_step_size = 2.0f;
    const float density_threshold = 0.01f;
    ...
    // March through volume with adaptive step size
    while (t_total < max_distance && fmaxf(transmittance.x, fmaxf(transmittance.y, transmittance.z)) > 0.01f)
    {
        ...
        {
            step_size = fmaxf(min_step_size, fminf(max_step_size, base_step_size / density));
        }

        // Early ray termination if density is very low
        if (density < 0.001f)
        {
            t_total += step_size;
            continue;
        ...
        float sigma_t_step = sigma_t * step_size;
        float extinction = sigma_t_step * density;
        
        // Calculate transmittance for each channel separately
        float trans_x = expf(-extinction);
        float trans_y = expf(-extinction);
        float trans_z = expf(-extinction);
        vec3 step_transmittance = vec3{trans_x, trans_y, trans_z};
        ...
        if (density > density_threshold)
        {
            ...
            for (int i = 0; i < num_lights; i++)
            {
                ...
                float light_dist = length(lights[i].position - p);

                // Skip if light is too far
                if (light_dist > 50.0f)
                    continue;

                // Calculate light contribution with improved attenuation
                float attenuation = 1.0f / (1.0f + light_dist * light_dist * atten_k);
                float phase = 1.0f; // Isotropic phase function

                // Accumulate light contribution
                vec3 light_contrib = lights[i].col * lights[i].intensity * 
                                   attenuation * phase * density * sigma_s;
                in_scattering = in_scattering + light_contrib;
            }

            // Add in-scattering contribution
            color = color + transmittance * in_scattering * step_size;
        }
    ...
    int nz)
{
    ...
    //     illumination = render_volume_kdtree(ray_origin, ray_direction, min, max, t_box, d_density_grid, nx, ny, nz, center, d_lighting_grids, num_lighting_grids, voxel_size);

    // if (illumination != vec3{0.0f})
    //     return illumination;

    return illumination; // cornellBox(ray_origin, ray_direction, t, lights, num_lights, normal, min, max, d_density_grid, nx, ny, nz, center);
}
